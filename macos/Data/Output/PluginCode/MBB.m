classdef MBB < audioPlugin
    ...
    properties ( Access = private )
        n;
        pos_Vin = [ 19 ];
        pos_R = [ 1 2 4 5 6 8 10 12 13 14 17 20 21 23 25 ];
        pos_C = [ 7 9 11 15 16 18 22 24 ];
        pos_Dap = [ 3 ];
        B_V; B_I;
        P; O;
        Z; S; 
    ...
    methods
        ...
        function updateP1( p )
            p.Z( 21 , 21 ) = p.x_1 * p.Rp( 1 ) + p.R_tol;
            p.Z( 6 , 6 ) = ( 1 - p.x_1 ) * p.Rp( 1 ) + p.R_tol;
        end

        function updateP2( p )
            p.Z( 25 , 25 ) = p.x_2 * p.Rp( 2 ) + p.R_tol;
            p.Z( 12 , 12 ) = ( 1 - p.x_2 ) * p.Rp( 2 ) + p.R_tol;
        end
